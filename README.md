...
No PowerPoint template is included in this repository. Slide generation loads a file named `template.pptx` from the `backend/` directory, so add your own before using that feature — any `.pptx` file works, and its master slides define the look of the output. Without it the app still runs and falls back to generating slides without a template.
...
Interface text is in Finnish and English.